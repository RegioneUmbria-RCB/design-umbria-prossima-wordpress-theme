(function (factory) {
  ...
  L.Control.Geocoder = L.Control.extend({
    ...
    _createMarker: function (result) {
      ...
      this._geocodeMarker = new L.Marker(
        result.center || result.target.getLatLng(),
        { draggable: true }
      )
        .bindPopup(
          "<span class='leaflet-map__popup'>" +
            result.center.lat +
            ", " +
            result.center.lng +
            "</span>"
  ...
  L.Control.Geocoder.jsonp = function (
    url,
    params,
    callback,
    context,
  ...
  L.Control.Geocoder.Mapbox = L.Class.extend({
    ...
    geocode: function (query, cb, context) {
      L.Control.Geocoder.getJSON(
        this.options.service_url + encodeURIComponent(query) + ".json",
        {
