<?php
    ...
    function generate_webp_in_separate_folder($metadata, $attachment_id) {
        ...
        foreach ($additional_sizes as $size_name => $size_attrs) {
            $resized_image = image_make_intermediate_size($original_file_path, $size_attrs['width'], $size_attrs['height'], $size_attrs['crop']);
            ...
            if ($resized_image && !is_wp_error($resized_image)) {
                ...
                $metadata['sizes'][$size_name]['file'] = pathinfo($resized_image['file'], PATHINFO_FILENAME) . '.webp';
            }
        }
