jQuery(document).ready(function () {
  ...
  jQuery('form[name="post"]').on("submit", function (e) {
     ...
     */
    if (
      document.activeElement.id === "publish" &&
      jQuery('input[name^="_dci_notizia_argomenti"]:checked').length == 0
    ) {
      dci_highlight_missing_field(".cmb2-id--dci-notizia-argomenti");
      return false;
    }
    inputDataPubblicazione = jQuery(
      'input[name^="_dci_notizia_data_pubblicazione"]'
    ).val();
    check_pubblicato = jQuery("#timestamp b").text();
    if (document.activeElement.id === "publish") {
      if (!inputDataPubblicazione) {
        // Verifica sia null che stringa vuota
        ...
        // Calcolo della data
        const dataDaRiportare =
          check_pubblicato === "subito"
            ? getCurrentDate()
            : parseDateString(check_pubblicato);

        // Imposta il valore nel campo
        jQuery('input[name^="_dci_notizia_data_pubblicazione"]').val(
          dataDaRiportare
        );
      }
    }
...
function dci_highlight_missing_field(fieldClass) {
  jQuery(fieldClass)
    ...
    .append(
      '<div id ="field-required-msg" class="field-required-msg"><em>Campo obbligatorio</em></div>'
    );
